...
from app.models import SentCouponCampaignSnapshot
from app.snapshot_retention import DEFAULT_RETAIN_HOURS, prune_snapshot_dicts

logger = logging.getLogger(__name__)

ROOT_KEY = "sent_coupon_campaigns"


class CouponCampaignPersistence:
    def __init__(
        self,
        file_path: str,
        *,
    ...
    def _read_data(self) -> dict:
        ...
        try:
            ...
            logger.error(
                "Histórico de cupons inválido em %s (%s); reiniciando vazio",
    ...
    def load_snapshots(self) -> list[SentCouponCampaignSnapshot]:
        ...
        if len(pruned) != len(raw_items):
            ...
            logger.info(
                "Histórico de cupons podado: %s -> %s (retain=%sh)",
                len(raw_items),
                len(pruned),
                self._retain_hours,
            )
        ...
        for item in pruned:
            if not isinstance(item, dict):
                continue
            try:
                snapshots.append(SentCouponCampaignSnapshot(**item))
            except TypeError as exc:
                logger.warning(
                    "Snapshot de cupom inválido ignorado em %s: %s",
                    self._file_path.name,
                    exc,
                )
        return snapshots

    def add_snapshot(self, snapshot: SentCouponCampaignSnapshot) -> None:
        data = self._read_data()
        raw_items = list(data.get(ROOT_KEY, []))
        pruned = prune_snapshot_dicts(raw_items, self._retain_hours)
